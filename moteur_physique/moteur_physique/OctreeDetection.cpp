...
//Creation d'octree a partir d'un transform qui sera notre racine | dans un octree, nous iterons recursivement jusqu'a associe a chaque donnée une region.
OctreeDetection::OctreeDetection(RigidBody* rb)
{
	m_center = rb->transform->getPosition();
	if (rb->primitive->shape.shape == "Sphere") {
		m_radius = rb->primitiveSphere->getRadius();
	}
	else if (rb->primitive->shape.shape == "Boite") {
...
OctreeNode* OctreeNode::BuildOctree(Vector3D center, float halfWidth, int maxDepth)
{
	if (maxDepth < 0) return nullptr;
		...
		Vector3D offset;
		float step = m_halfWidth * .5f;//on divise par 2 recursivement pour obtenir des regions de plus en plus petites et donc discretiser au maximum l'espace

		for (int i = 0; i < 8; i++) { 
			//les opérations sur les bits ci-dessous nous permettent d'inverser ou non les valeurs x, y, z du centre d'une region. 
			//l'arbre est sous la forme :
			//developper ici
...
{
	std::string shapeA = a->m_rb->primitive->shape.shape;
	std::string shapeB = b->m_rb->primitive->shape.shape;
	
	if (shapeA == "Sphere") {
		if (shapeB == "Sphere") {
			ContactResolutions::CollisionSphereSphere(*a->m_rb->primitiveSphere, *b->m_rb->primitiveSphere, collisionData);
			return;
		}
		else if (shapeB == "Plan") {
			ContactResolutions::CollisionSpherePlan(*a->m_rb->primitiveSphere, *b->m_rb->primitivePlan, collisionData);
			return;
		}
		else if (shapeB == "Boite") {
			ContactResolutions::CollisionBoiteSphere(*a->m_rb->primitiveSphere, *b->m_rb->primitiveBoite, collisionData);
			return;
		}
	}
	if (shapeA == "Plan") {
		if (shapeB == "Sphere") {
			ContactResolutions::CollisionSpherePlan(*b->m_rb->primitiveSphere, *a->m_rb->primitivePlan, collisionData);
			return;
		}
		else if (shapeB == "Plan") {
	...
	if (shapeA == "Boite") {
		if (shapeB == "Sphere") {
			ContactResolutions::CollisionBoiteSphere(*b->m_rb->primitiveSphere, *a->m_rb->primitiveBoite, collisionData);
			return;
		}
		else if (shapeB == "Plan") {
