...
public:
	...
	RigidBody* CreerCube(Vector3D cubePosition, Vector3D cubeVelocity, int index)
	{
		...
		//bien sur, si nous voulons un autre type de primitive, nous pouvons faire un enum et rentrer en parametre ce que nous souhaitons pour avoir Boite/Plan/Sphere
		Boite* boite = new Boite();//AJOUTER DES ATTRIBUTS 
		Plan* plan = new Plan();
