...
public:
	...
	void CreerContacts(Particle* newParticule)
	{
		for (Particle* particule : listeParticules)
		{
			if (particule != newParticule)
			{
				...
				listeParticulesContacts.push_back(new ParticleContact(duoParticles));//par defaut restitution � 0.5 et penetration � 0
			}
		}
	...
	void UpdateVariousFrameRate(bool pause, float duration)
	{
		if (pause)
		{
			...
			//Actualisation des forces appliqu�es � nos particules
	...
	void CreerGenerateurForce(int type, Particle* particule, float listParamsForceGenerator[] = {}, Particle* otherParticule = new Particle())//0 Buoancy, 1 drag, 2 gravity, 3 spring
	{
		...
		case 3:
		{
			...
			{
				std::cout << "Impossible d'ajouter un particle spring avec une seconde particule non initialis�e.\n";
			}
		}
		default:
		{
